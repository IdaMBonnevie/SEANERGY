...
def calculatePotentialSynergiesWithExistingUsesRaster(marineUseInFocus, dictMarineUses, matrixexcelfile, inputfolder, final_folder, pairwise_output_folder, process_folder, addConflictingExistingMarineUseInteractionsIsTrue, synergy_with_existing_uses_raster_only_ocean_name, oceanrastername_with_path, lower_left_corner, dsc, spatial_ref, starttimetobeupdated):
    ...
    if marineUseInFocus in listOfLongMarineUseNames:
        calculationtype = "pot_count_"+dictMarineUses.get(marineUseInFocus)[:-4]+"_"
        inMarineUseRasterDict = True
    else: 
        calculationtypepart = marineUseInFocus.replace(" ","")
        if len(calculationtypepart) > 7:
            calculationtype = "pot_count_"+marineUseInFocus[:7]+"_"
            marineusename = marineUseInFocus[:7]
        else: 
            calculationtype = "pot_count_"+marineUseInFocus+"_"  
    ...
    iterator1 = 0
    iterator2 = 1
    #make total potential count raster of potential synergy overlaps between existing marine uses and the chosen new marine use, as well as make individual marine use count polygons of marine uses with potential synergies with the chosen new marine use: 
    for column in listOfLongMarineUseNames:  
                        ...
                        if len(str(alldesc)) >= 4: 
                            ...
                            if spec_score < 0: 
                                if (marineUseInFocus == column):
                                    (count_outras_conflict_array, overnull, marineusepolygonlist, marineusepolygonnamelist) = countToOneRaster(inputfolder, inputrastername2, lower_left_corner, dsc, spatial_ref, alldesc, process_folder, marineusepolygonlist, marineusepolygonnamelist, calculationtype)
                                elif (marineUseInFocus == listOfLongMarineUseNames[iterator2]):
                                    (count_outras_conflict_array, overnull, marineusepolygonlist, marineusepolygonnamelist) = countToOneRaster(inputfolder, inputrastername1, lower_left_corner, dsc, spatial_ref, alldesc, process_folder, marineusepolygonlist, marineusepolygonnamelist, calculationtype)
                                if firstnegraster == True:         
                                    remove_area_array = count_outras_conflict_array > 0
                                    firstnegraster = False
                                else: 
                                    specific_count_array = np.copy(count_outras_conflict_array)
                                    remove_area_specific_array = specific_count_array > 0
                                    remove_area_array = (remove_area_array | remove_area_specific_array)
                            if spec_score > 0: 
                                if (marineUseInFocus == column):
                                    (count_outras_array, overnull, marineusepolygonlist, marineusepolygonnamelist) = countToOneRaster(inputfolder, inputrastername2, lower_left_corner, dsc, spatial_ref, alldesc, process_folder, marineusepolygonlist, marineusepolygonnamelist, calculationtype)
                                    if overnull == True: 
                                        listOfExistingMarineUses.append(inputrastername2)
                                elif (marineUseInFocus == listOfLongMarineUseNames[iterator2]):
                                    (count_outras_array, overnull, marineusepolygonlist, marineusepolygonnamelist) = countToOneRaster(inputfolder, inputrastername1, lower_left_corner, dsc, spatial_ref, alldesc, process_folder, marineusepolygonlist, marineusepolygonnamelist, calculationtype)
                                    if overnull == True: 
                                        listOfExistingMarineUses.append(inputrastername1)                                    
                                if firstposraster == True:         
                                    cummulated_count_outras_array = count_outras_array
        ...
        ### the pairwise count is found for the column and the marine-use-in-focus-as-row
        ...
        ### the pairwise count is found iteratively for the marine-use-as-column and other-marine-uses-as-row -
    ...
    np.place(cummulated_count_outras_array, remove_area_array, -1) #highlight conflicts
    np.place(cummulated_count_outras_array, ocean_array_land, np.nan)
    scoreRaster = arcpy.NumPyArrayToRaster(cummulated_count_outras_array, lower_left_corner, dsc.meanCellWidth, dsc.meanCellHeight)
    ...
    #make pairwise marine use count polygons if any negative scores between existing marine uses exist:    
            ...
            while raster1iterator < len(listOfExistingMarineUses):
                ...
                for raster2iterator in range(raster1iterator+1, len(listOfExistingMarineUses)):
                    ...
                    if len(alldesc_nparray) > 0:
                        ...
                        if len(str(alldesc)) >= 4: 
                            if alldesc[:1] == "-": 
                                (pairwisescorepolygonlist, pairwisescorepolygonnamelist) = countToRasters(inputfolder, shortrastername1, shortrastername2, lower_left_corner, dsc, alldesc, pairwise_output_folder, pairwisescorepolygonlist, pairwisescorepolygonnamelist, "count_")
                raster1iterator += 1
...
def individualOrPairwiseScoreRasterToPolygon(array_input, lower_left_corner, dsc, spatial_ref, output_folder, inputrastername1, inputrastername2, calculationtype, matrixinput, polygonlist, polygonnamelist):
...
def countToRasters(inputfoldername, inputrastername1, inputrastername2, lower_left_corner, dsc, matrixinput, pairwise_output_folder, pairwisemarineusepolygonlist, pairwisemarineusepolygonnamelist, calculationtype):
    ...
    if np.sum(array_input1) > 0: 
        (pairwisemarineusepolygonlist, pairwisemarineusepolygonnamelist) = individualOrPairwiseScoreRasterToPolygon(array_input1, lower_left_corner, dsc, spatial_ref, pairwise_output_folder, inputrastername1, inputrastername2, calculationtype, matrixinput, pairwisemarineusepolygonlist, pairwisemarineusepolygonnamelist)
    return (pairwisemarineusepolygonlist, pairwisemarineusepolygonnamelist)

def countToOneRaster(inputfoldername, inputrastername, lower_left_corner, dsc, spatial_ref, matrixinput, process_folder, marineusepolygonlist, marineusepolygonnamelist, calculationtype):
    #create individual marine use count raster
    ...
    if np.sum(array_input) > 0: 
        overnull=True
        (marineusepolygonlist, marineusepolygonnamelist) = individualOrPairwiseScoreRasterToPolygon(array_input, lower_left_corner, dsc, spatial_ref, process_folder, inputrastername, inputrastername, calculationtype, matrixinput, marineusepolygonlist, marineusepolygonnamelist)
